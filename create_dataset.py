...
# output: tensor
def create_complete_set_traces_one_true_literal(length_traces, alphabet, dfa, verbose=False):
    traces = []
    traces_t = []
    accepted_list = []
    ...
    traces_t_train = torch.stack(traces_t)
    accepted_train = torch.stack(accepted_list)



    if verbose:
        train_size = traces_t_train.size()[0]
        print("created symbolic dataset with all the {} traces of length {} composed by {} symbols; ".format(train_size, length_traces, len(alphabet)))

    return [traces_t_train], [accepted_train]
...
#-------------------------------------------------------------------------is a list of legnths!-----------------------
def create_complete_set_traces_one_true_literal_2(max_length_train_traces, lengthS_test_traces, alphabet, dfa, batch_size=64, verbose=False):
    dataset_X_train = []
    dataset_y_train = []

    dataset_X_test = [[] for _ in lengthS_test_traces]
    dataset_y_test = [[] for _ in lengthS_test_traces]
    ...
    for length_traces in list(range(1,max_length_train_traces +1)) + lengthS_test_traces:
        prod = product(alphabet, repeat=length_traces)
        traces_t = []
        accepted_list = []
        ...
        positives = [x for x, y in zip(traces_t, accepted_list) if y == 1]
        negatives = [x for x, y in zip(traces_t, accepted_list) if y == 0]

        #shuffle data of the same length
        if length_traces in lengthS_test_traces:
            k = lengthS_test_traces.index(length_traces)
            min_len = min(len(positives), len(negatives))
            random.shuffle(positives)
            random.shuffle(negatives)
            selected_positives = positives[:min_len]
            selected_negatives = negatives[:min_len]

            dataset_X_test[k] += selected_positives + selected_negatives
            dataset_y_test[k] += [1] * min_len + [0] * min_len

            all_test += 2 * min_len
            positive_test += min_len
        else:
            dataset_X_train += traces_t

            dataset_y_train += accepted_list
    ...
    if verbose:
        ...
        for k in range(len(lengthS_test_traces)):
            print("Test dataset ", k)
            for i in range(len(dataset_X_test[k])):
                print("test batch {}___________".format(i))
                print(dataset_X_test[k][i].size())
                #print(dataset_y_test[k][i].size())
                print(dataset_X_test[k][i])
                print(dataset_y_test[k][i])
...
def create_image_sequence_dataset(image_data, numb_of_classes, traces, acceptance, batch_size=64, print_size=False):
    ...
    seen_one_complete_trace_batch = False
    while True:
        for j in range(len(traces)):
            ...
            for step in range(num_img):
                if x[step][0] > 0.5:
                    ...
                    x_i_img[step] = data_for_classes[0][i_i[0] % how_many[0]]
                    i_i[0] += 1

                else:
                    x_i_img[step] = data_for_classes[1][i_i[1] % how_many[1]]
                    i_i[1] += 1

            if (i_i[0] >= how_many[0] or i_i[1] >= how_many[1]) and seen_one_complete_trace_batch:
                break
        ...
        if (i_i[0] >= how_many[0] or i_i[1] >= how_many[1]) and seen_one_complete_trace_batch:
            ...
            seen_one_complete_trace_batch = True
    ...
    final_dataset = []
    for batch_id in range(len(img_seq_train_batch)):

        batch_X = img_seq_train_batch[batch_id]
        batch_y = acceptance_train_batch[batch_id]
        ...
        final_dataset += [
            (torch.stack(batch_X[i:i + batch_size]), torch.LongTensor(batch_y[i:i + batch_size]))
            for i in range(0, len(batch_X), batch_size)
        ]

    if print_size:
        print("Created image dataset with {} sequences of images".format(sum([batch[0].size()[0] for batch in final_dataset])))
