...
class MinimizableMooreMachine():
    ...
    def apply_symbols_map(self, map):
        ...
        #NO MINIMIZATION
        '''
        self.alphabet = list(set(map))
        self.num_of_symbols = len(self.alphabet)
        

        pyautomaton = transacc2pythomata(self.transitions, self.acceptance, self.alphabet)
        pyautomaton = pyautomaton.reachable()
        pyautomaton = pyautomaton.minimize()
        self.pydfa = pyautomaton
...
def minimize_dfa_symbols_and_states(pythomata_dfa):
    ...
    len_urs = []
    '''
    print("Unremovable reasoning shortcuts")
    for rs in urs:
            len_urs.append(len(set(rs)))
            print(rs)
    print("--------------")
    '''
    if len_urs == []:
...
'''
def minimized_deepDFA(minpydfa):
    print(minpydfa.__dict__)
    num_states = len( minpydfa._states)
    num_symbols = len( minpydfa._idx_to_symbol)
    
    deepdfa = ProbabilisticAutoma(num_states, num_symbols, 2, minpydfa._initial_state)
    
    deepdfa.initFromPydfa(minpydfa, 100)
    assert False

from declare_formulas import formulas_names
from statistics import mean, stdev
all_num_sym = []

#dir = "learning_all_experiment/no_bias/small_dataset_hd=20_nsym=4_NO_trans_cut/"
dir = "learning_all_experiment/no_bias/medium_ds_hd=25_ns=4_TRANS_CUT/"
num_sym_f = open(dir+"Plots/symbols_count.txt", 'a')
...
for formula in formulas_names:
    ...
    for exp in range(10):
        print("Exp: ", exp)
        pckl_path = dir+"Automata/{}_exp{}_minimized.pcl".format(formula, exp)
        with open(pckl_path, "rb") as f:
            pydfa = pickle.load(f)

        min_pydfa, deleted_syms, transformed_alphabet = minimize_dfa_symbols_and_states(pydfa)
        print("DELETED SYMS: ", deleted_syms)
        print("TRANSOFORMED ALPHABET: ", transformed_alphabet)
        num_sym.append(len(min_pydfa.alphabet))
        min_pydfa.to_graphviz().render(dir+"Automata/" + formula + "_exp" + str(exp) + "_DOUBLEminimized.dot")

        #### from pydfa to deepdfa
        #transformation_matrix, filter_vector = create_mask_classifier(deleted_syms, transformed_alphabet)

    num_sym_f.write("{}+-{}\n\n".format(mean(num_sym), stdev(num_sym)))
    all_num_sym += num_sym

num_sym_f.write("ALL FORMULAS:\n")
num_sym_f.write("{}+-{}\n\n".format(mean(all_num_sym), stdev(all_num_sym)))
num_sym_f.close()
