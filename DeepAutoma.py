...
class ProbabilisticAutoma(nn.Module):
    ...
    def __init__(self, numb_of_actions, numb_of_states, numb_of_rewards, initial_state = 0):
        ...
        self.trans_prob = torch.normal(0, 0.5, size=( numb_of_actions, numb_of_states, numb_of_states), requires_grad=False, device=device)

        self.rew_matrix = torch.normal(0, 0.5, size=( numb_of_states, numb_of_rewards), requires_grad=False, device=device)
    ...
    def initFromDfa(self, reduced_dfa, outputs, trans_weight=10, rew_weight=10):
        with torch.no_grad():
        ...
        for s in reduced_dfa:
            for a in reduced_dfa[s]:
                with torch.no_grad():
                    self.trans_prob[a, s, reduced_dfa[s][a]] = trans_weight

        #set reward matrix
        for s in range(len(reduced_dfa.keys())):
                with torch.no_grad():
                    self.rew_matrix[s, outputs[s]] = rew_weight
    ...
    def next_state(self,state, action):
        ...
        for s in self.dfa.keys():
            for sym in self.dfa[s].keys():
                ...
                #nxt_stt[self.dfa[s][sym]] = disjunction( nxt_stt[self.dfa[s][sym]], conjunction(state[s], action[sym]))
